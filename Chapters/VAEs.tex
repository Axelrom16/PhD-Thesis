...
\end{equation}
where we have an intractability problem due to the marginal likelihood $p_{\theta}(\mathbf{x})$ which is given by an integral over the latent variables (eq \ref{eq:p_x}). 
...
distribution, typically a Gaussian distribution $p_{\theta}(\mathbf{z}) = \mathcal{N}(\mathbf{z}; \mathbf{0}, \mathbf{I})$, which is easy to sample from. \\
...
\begin{itemize}
    ...
    To mitigate this, a common solution proposed by \cite{bowman2016generatingsentencescontinuousspace} and \cite{sonderby2016howtrain} is to implement an optimization schedule (KL annealing) where the weight of the latent cost 
...
and decoder must be trained jointly and learning becamoes unstable. A clever twist to solve the main problems and maintain the hierarchical structure is the \textbf{Denoising Diffusion Probabilistic Models} (DDPMs)
\cite{sohldickstein2015deepunsupervisedlearningusing, ho2020denoisingdiffusionprobabilisticmodels}, the next step in generative modeling.  
...
\begin{summarybox}[label={box:latent_math}]{VAEs and DDPMs}
    ...
    \medskip
    \textbf{3. Training Objective of Gaussian VAEs:}
    \begin{align}
        &\mathcal{L}_{ELBO}(\theta, \phi; \mathbf{x}) = -\frac{1}{2\sigma^2} \mathbb{E}_{\mathbf{z}\sim q_{\phi}(\mathbf{z}|\mathbf{x})} \left[||\mathbf{x} - \mu_{\theta}(\mathbf{z})||^2\right] \\ 
        &- \frac{1}{2} \sum_{i=1}^k \left( \sigma^2_{\phi,i}(\mathbf{x}) + \mu^2_{\phi,i}(\mathbf{x}) - 1 - \log \sigma^2_{\phi,i}(\mathbf{x}) \right) + C.
    \end{align} 
